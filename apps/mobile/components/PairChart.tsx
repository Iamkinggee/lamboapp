...
// LOCATION: apps/mobile/components/PairChart.tsx
// FIX: Added fullscreen mode — tap the expand button to fill the entire screen.
// Chart now uses Modal for true fullscreen, making it usable for real analysis.
    ...
    >
      ...
      <SafeAreaView style={fsStyles.container}>
        ...
        <View style={fsStyles.header}>
          <Text style={fsStyles.title}>{pair.replace('USDT', '/USDT')}</Text>
          <TouchableOpacity onPress={onClose} style={fsStyles.closeBtn} activeOpacity={0.7}>
            <Text style={fsStyles.closeBtnText}>✕ Close</Text>
          </TouchableOpacity>
        </View>

        {/* Chart */}
        <View style={fsStyles.chartWrap}>
          {loading && !error && (
            <View style={fsStyles.loadingOverlay}>
              <ActivityIndicator color={Colors.accent} size="large" />
              <Text style={fsStyles.loadingText}>Loading chart...</Text>
            </View>
          )}
          {error ? (
            <View style={fsStyles.errorWrap}>
              <Text style={{ fontSize: 36 }}>📡</Text>
              <Text style={fsStyles.errorText}>Chart unavailable</Text>
            </View>
          ) : (
            <WebView
              source={{ html }}
...
const fsStyles = StyleSheet.create({
  ...
  closeBtn:     { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, backgroundColor: '#1a1a1a' },
  ...
  errorText:    { fontSize: 16, fontWeight: '700', color: '#fff' },
});
          ...
          {error ? (
            ...
            <>
              <WebView
                ...
                originWhitelist={['*']}
              />
              {/* Fullscreen button — top right of chart */}
              {!loading && (
                <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  fullscreenBtnText: { fontSize: 16, color: '#fff' },
